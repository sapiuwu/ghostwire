...
import type { ScreenSize } from '../screen/frame.js';
...
function parseScreen(obj: Record<string, unknown>): ScreenSize {
...
export function decodeHello(payload: Uint8Array): HelloPayload {
...
export function encodeWelcome(payload: WelcomePayload): Uint8Array {
  const out: Record<string, unknown> = { ok: payload.ok };
  if (payload.reason !== undefined) out['reason'] = payload.reason;
  if (payload.serverVersion !== undefined) out['serverVersion'] = payload.serverVersion;
  if (payload.screen !== undefined) out['screen'] = payload.screen;
  if (payload.fps !== undefined) out['fps'] = payload.fps;
  return encoder.encode(JSON.stringify(out));
}

export function decodeWelcome(payload: Uint8Array): WelcomePayload {
  const obj = parseObject(payload);
  const ok = obj['ok'];
  ...
  const out: WelcomePayload = { ok, screen: parseScreen(obj) };
  if (reason !== undefined) out.reason = reason;
  if (serverVersion !== undefined) out.serverVersion = serverVersion;
  if (fps !== undefined) out.fps = fps;
  return out;
}

export function encodePing(payload: PingPayload): Uint8Array {
  return encoder.encode(JSON.stringify({ t: payload.t }));
}

export function decodePing(payload: Uint8Array): PingPayload {
  const obj = parseObject(payload);
...
export function encodeBye(payload: ByePayload): Uint8Array {
  return encoder.encode(JSON.stringify({ reason: payload.reason }));
}

export function decodeBye(payload: Uint8Array): ByePayload {
  const obj = parseObject(payload);
  return { reason: requireString(obj, 'reason', 256) };
}

export function encodeScreenInfo(payload: ScreenInfoPayload): Uint8Array {
  return encoder.encode(JSON.stringify({ width: payload.width, height: payload.height }));
}

export function decodeScreenInfo(payload: Uint8Array): ScreenInfoPayload {
  const obj = parseObject(payload);
