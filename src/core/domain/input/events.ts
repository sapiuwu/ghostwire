...
export type InputEvent =
  | { kind: 'key'; key: number; down: boolean; modifiers: number }
  | { kind: 'mouseMove'; x: number; y: number; modifiers: number }
  | { kind: 'mouseButton'; button: MouseButtonValue; down: boolean; x: number; y: number; modifiers: number }
  | { kind: 'wheel'; x: number; y: number; deltaY: number; modifiers: number }
  | { kind: 'text'; code: number };
...
export function decodeInputEvent(data: Uint8Array): InputEvent {
  if (data.length !== 20) {
  ...
  switch (kind) {
    ...
    case InputKind.MouseButton:
      return { kind: 'mouseButton', button: code as MouseButtonValue, down, x, y, modifiers };
