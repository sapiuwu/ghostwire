...
export function keyPair(key, modifiers) {
    return [
        { kind: 'key', key, down: true, modifiers },
        { kind: 'key', key, down: false, modifiers },
    ];
}
export function keyFromChar(ch, baseModifiers = 0) {
    if (ch.length !== 1)
        return null;
    const code = ch.charCodeAt(0);
    if (ch >= 'a' && ch <= 'z')
        return { key: Key.A + (code - 97), modifiers: baseModifiers };
    if (ch >= 'A' && ch <= 'Z')
        return { key: Key.A + (code - 65), modifiers: baseModifiers | Modifier.Shift };
    if (ch >= '0' && ch <= '9')
        return { key: Key.Digit0 + (code - 48), modifiers: baseModifiers };
    if (ch === ' ')
        return { key: Key.Space, modifiers: baseModifiers };
    const shifted = SHIFTED_SYMBOLS[ch];
    if (shifted !== undefined)
        return { key: shifted, modifiers: baseModifiers | Modifier.Shift };
    const plain = PLAIN_SYMBOLS[ch];
    if (plain !== undefined)
        return { key: plain, modifiers: baseModifiers };
    return null;
}
function decodeXtermMods(value) {
...
function matchesAt(buf, index, seq) {
    if (index + seq.length > buf.length)
        return false;
    for (let i = 0; i < seq.length; i++) {
        if (buf[index + i] !== seq[i])
            return false;
    }
    return true;
}
function decodeUtf8(buf, index, flush) {
...
function parsePlain(buf, from, to, options, flush) {
    const events = [];
    let i = from;
    while (i < to) {
        const byte = buf[i];
        ...
        if (byte < 0x80) {
            ...
            if (mapped)
                events.push(...keyPair(mapped.key, mapped.modifiers));
            i++;
            continue;
        }
        ...
        if (!decoded)
            break;
        if (decoded.code >= 0x20)
            pushText(events, decoded.code);
        i = decoded.next;
    }
    return { events, consumed: i };
}
function parseSgrMouse(params, press, options) {
    const parts = params.slice(1).split(';');
    const buttonBits = Number(parts[0]);
    const cellX = Number(parts[1]);
    const cellY = Number(parts[2]);
    if (!Number.isFinite(buttonBits) || !Number.isFinite(cellX) || !Number.isFinite(cellY))
        return [];
    let modifiers = 0;
    if ((buttonBits & 4) !== 0)
    ...
    return [{ kind: 'mouseButton', button: mappedButton, down: press, x: pos.x, y: pos.y, modifiers }];
}
function parseCsi(buf, start, limit, options, flush) {
    let i = start + 1;
    while (i < limit) {
        const byte = buf[i];
    ...
    if ((final === 'I' || final === 'O') && params === '')
        return { events: [], next };
    if (final === 'Z')
        return { events: keyPair(Key.Tab, Modifier.Shift), next };
...
function parseEscape(buf, start, limit, options, flush) {
    if (start + 1 >= limit) {
        if (flush)
    ...
    if (nextByte === 0x5d) {
        ...
        while (i < limit) {
            ...
            if (byte === 0x1b && i + 1 < limit && buf[i + 1] === 0x5c)
                return { events: [], next: i + 2 };
            i++;
        }
        if (flush)
            return { events: [], next: limit };
    ...
    if (nextByte < 0x20) {
        if (nextByte === 0x09)
            return { events: keyPair(Key.Tab, Modifier.Alt), next: start + 2 };
        if (nextByte === 0x0d || nextByte === 0x0a)
            return { events: keyPair(Key.Enter, Modifier.Alt), next: start + 2 };
    ...
    if (nextByte < 0x80) {
        ...
        if (mapped)
            return { events: keyPair(mapped.key, mapped.modifiers), next: start + 2 };
        return { events: [], next: start + 2 };
    }
...
function findSequence(buf, from, limit, seq) {
...
export function parseTerminalInput(buf, options, flush = false) {
