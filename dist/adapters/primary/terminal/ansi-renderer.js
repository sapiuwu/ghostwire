...
export class AnsiRenderer {
    ...
    sgrFg = -1;
    sgrBg = -1;
    constructor(io, cols, rows, statusEnabled = true) {
        this.io = io;
        this.cols = cols;
        this.rows = rows;
        ...
        for (let row = 0; row < renderRows; row++) {
            ...
            while (col < cols) {
                ...
                while (col < cols) {
                    ...
                    const fg = nextFg[i];
                    const bg = nextBg[i];
                    if (fg === this.curFg[i] && bg === this.curBg[i])
                        break;
                    out += this.sgr(fg, bg);
                    out += HALF_BLOCK;
                    this.curFg[i] = fg;
    ...
    status(text) {
        if (!this.statusEnabled)
            return;
        const row = this.rows;
        this.io.write(`\x1b[${row};1H\x1b[2K\x1b[38;2;170;187;204m${text}\x1b[0m`);
        this.sgrFg = -1;
        this.sgrBg = -1;
    }
    end() {
        this.io.write('\x1b[?1006l\x1b[?1003l\x1b[?1002l\x1b[?1000l' +
...
function averageColor(pixels, x0, y0, x1, y1, stride) {
    let sumB = 0;
    let sumG = 0;
    let sumR = 0;
    let count = 0;
