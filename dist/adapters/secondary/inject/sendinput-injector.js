...
function loadInputApi() {
    ...
    cachedApi = {
        ...
        sendInput: (inputs) => SendInput(inputs.length, inputs, koffi.sizeof(INPUT)),
        inputSize: koffi.sizeof(INPUT),
    };
    return cachedApi;
}
function isExtendedKey(vk) {
    return EXTENDED_KEYS.has(vk);
}
export class SendInputInjector {
    heldModifiers = 0;
    inject(event) {
        ...
        if (sent !== inputs.length) {
            throw new GhostwireError('internal', `SendInput delivered ${sent} of ${inputs.length} events (blocked by UIPI or another input consumer)`);
        }
    }
    releaseAll() {
        if (!cachedApi || this.heldModifiers === 0)
            return;
        const api = cachedApi;
        const release = MODIFIER_KEYS.filter((mod) => (this.heldModifiers & mod.bit) !== 0).map((mod) => this.keyInput(mod.vk, false));
    ...
    mouseInput(api, flags, mouseData, x, y) {
...
function buttonFlags(button, down) {
    switch (button) {
        case 1:
...
//# sourceMappingURL=sendinput-injector.js.map