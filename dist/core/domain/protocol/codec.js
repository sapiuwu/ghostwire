...
import { HEADER_FLAG, HEADER_SIZE, MAX_PAYLOAD_SIZE, PROTOCOL_MAGIC, PROTOCOL_VERSION, } from './constants.js';
const CONTROL_COMPRESS_THRESHOLD = 256;
export function encodeMessage(kind, payload, flags = 0) {
...
export function encodeJSONMessage(kind, value) {
...
export function decodeControlPayload(message) {
    if ((message.flags & HEADER_FLAG.Deflate) === 0)
...
export class MessageDecoder {
    ...
    buffered = 0;
    feed(chunk) {
        const buffer = chunk instanceof Buffer
            ? chunk
        ...
        for (;;) {
            ...
            if (this.buffered < HEADER_SIZE + length)
                break;
