...
import { FrameEncoding, MessageKind, PROTOCOL_VERSION, } from '../domain/protocol/constants.js';
...
import { decodeBye, decodeError, decodePing, decodeScreenInfo, decodeWelcome, } from '../domain/protocol/messages.js';
...
export class ClientService {
    ...
    async connect(options) {
        ...
        const finish = (err) => {
            ...
            wake();
            if (err)
                logger.debug('session ended with error', { error: err.message });
        };
        conn.onData((chunk) => {
            if (closed)
                return;
        ...
        const waitForInbox = async (deadlineAt) => {
            if (inbox.length > 0)
                return;
        ...
        const info = { screen: { width: 0, height: 0 }, fps: 0, serverVersion: '' };
        const stats = {
            framesReceived: 0,
            framesDropped: 0,
            bytesReceived: 0,
        ...
        const frameTimes = [];
        conn.write(encodeJSONMessage(MessageKind.Hello, {
...
function decodeByeSafe(message) {
    try {
